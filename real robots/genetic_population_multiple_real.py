...
class GeneticPopulation:
    ...
    def _receive_genome(self, msg):
        ...
        if not parameters.useLightSensors and (len(genomeAction) != parameters.genLengthAction or len(genomePrediction) != parameters.genLengthPred):  #check if genome is complete
            self.state = State.WAIT  
            print("receive fail")
            self.tmpGenome = received
            return False
        if parameters.useLightSensors and (len(genomeAction) != parameters.genLengthAction or len(genomePrediction) != parameters.genLengthPred):  #check if genome is complete
            self.state = State.WAIT
            print("receive fail")
            self.tmpGenome = received
            return False
