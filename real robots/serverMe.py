...
class Server:
    ...
    # dispatcher baut neue anfragen auf -> weist index nummern fuer die einzelnen clients zu
    def dispatcher(self):
    ...
    def on_new_client(self, clientsocket, addr, clientIndex):
        # if u want to use the values globaly just use global variables. PAY ATTENTION TO RACECONDITIONS
        print("connected to new Client... ")
        # sendet clientID
        s = "clientindex " + str(clientIndex)
        clientsocket.send(s.encode())

        # communication recieving from clients
        msg = ""
